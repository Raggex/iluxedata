...
const { actualizarConsultas } = require("../config/fx/consultas"); // Asegúrate de importar la función
...
*BUSCADO POR* →  ${userId}
        ...
        bot.sendMessage(chatId, mssg, messageOptions).then(async () => {
          await restarCreditos(userId, CREDITS);
          await activarAntiSpam(userId, ANTISPAM);
          await actualizarConsultas(userId, CONSULTAS); // Incrementa el conteo de consultas
        });
      } catch (error) {
